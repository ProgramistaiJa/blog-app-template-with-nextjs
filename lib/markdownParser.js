...
export const getFileBySlug = async (path, slug) => {
  ...
  let content = '';
  // let contentWithoutPrism = '';
  if (markdownContent) {
    // contentWithoutPrism = await remark().use(html).process(markdownContent);
    // contentWithoutPrism = contentWithoutPrism.toString();
    // console.log('contentWithoutPrism', contentWithoutPrism);
